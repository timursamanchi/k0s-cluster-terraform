...
resource "aws_launch_template" "workers_lt" {
  name_prefix            = "k0s-worker-" # Each template will have a unique name starting with this prefix
  ...
  key_name               = aws_key_pair.k0s_key_pair.key_name      # SSH key pair to access the worker nodes (used for debugging or manual operations)
  vpc_security_group_ids = [aws_security_group.k0s_worker_sg.id]   # Controls inbound/outbound traffic; should allow necessary ports (e.g., 10250 for kubelet, nodeport range, etc.)
  ...
  tag_specifications {
    ...
    tags = {
      Name = "k0s-worker" # Helps with easy identification in AWS console or CLI
...
resource "aws_autoscaling_group" "workers_asg" {
  name = "k0s-workers-asg-${random_string.asg_suffix.result}"
  # Auto-Scaling-Group name; random suffix ensures uniqueness across multiple deployments
