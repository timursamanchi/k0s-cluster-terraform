...
resource "aws_subnet" "public" {
  ...
  cidr_block              = cidrsubnet(aws_vpc.k0s.cidr_block, 4, count.index + (local.subnet_count * 2))
...
resource "aws_subnet" "controller_priv" {
  count             = local.subnet_count
  vpc_id            = aws_vpc.k0s.id
  cidr_block        = cidrsubnet(aws_vpc.k0s.cidr_block, 4, count.index)
  availability_zone = data.aws_availability_zones.available.names[count.index]
  tags = {
...
resource "aws_subnet" "worker_priv" {
  count             = local.subnet_count
  vpc_id            = aws_vpc.k0s.id
  cidr_block        = cidrsubnet(aws_vpc.k0s.cidr_block, 4, count.index + local.subnet_count)
  availability_zone = data.aws_availability_zones.available.names[count.index]
  tags = {
